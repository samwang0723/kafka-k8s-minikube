...
EOF
	...
	keytool -noprompt -keystore ${PFX}client.keystore.jks -exportcert -alias localhost \
		-rfc -storepass "$PASS" -file ssl/client_cert.pem

	### Extract client key
	keytool -noprompt -srckeystore ${PFX}client.keystore.jks -importkeystore -srcalias localhost \
		-destkeystore cert_and_key.p12 -deststoretype PKCS12 -srcstorepass "$PASS" -storepass "$PASS"

	openssl pkcs12 -in cert_and_key.p12 -nocerts -nodes -passin pass:${PASS} -out ssl/client_key.pem

	### Extract CA certificate
	keytool -noprompt -keystore ${PFX}client.keystore.jks -exportcert -alias CARoot -rfc -file ssl/ca_cert.pem \
		-storepass "$PASS"
    else
